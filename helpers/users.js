...
function validateUserSubscription(tagName) {
...
exports.validateUserSubscription = validateUserSubscription;
...
function processUpdate(tagName){
  getMaxUserID(tagName, function(error, maxUserID){
    helpers.instagram.tags.recent({ 
      name: tagName, 
      max_tag_id: maxUserID,
      complete: function(data,pagination) {
        setMaxUserID(tagName, pagination);
        var r = redis.createClient(settings.REDIS_PORT,settings.REDIS_HOST);
...
// This is User specific, the pagination objects
// seem to be inconsitant across subsciption channels

function getMaxUserID(tagName, callback){
  var r = redis.createClient(settings.REDIS_PORT, settings.REDIS_HOST);
  r.get('max-tag-id:channel:' + tagName, callback);
  r.quit();
}
exports.getMaxUserID = getMaxUserID;

function setMaxUserID(tagName, pagination){
  var r = redis.createClient(settings.REDIS_PORT, settings.REDIS_HOST);
  r.set('max-tag-id:channel:' + tagName, pagination.next_max_id);
  r.quit();
}
exports.setMaxUserID = setMaxUserID;